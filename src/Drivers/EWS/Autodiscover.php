...
class Autodiscover
{
    ...
    /**
     * The Exchange username to use during authentication.
     * If unspecified,
    ...
    /**
     * The Autodiscover XML request.
     * Since it's used repeatedly, it's cached
    ...
    /**
     * The Certificate Authority path.
    ...
    /**
     * The path to a specific Certificate Authority file.
     * Get one and use it
     ...
     * Bad idea, and violates the strict Autodiscover
     ...
     * last request.
     * Keys are lowercased for easy checking.
     *
     * @var array
     */
     ...
     *
     * @return string boolean known version constant, or FALSE if it could not
     *         be determined.
     *        
    ...
    public function newEWS()
    {
        ...
        if ($server) {
            ...
            return new ExchangeWebServices($server, $this->email, $this->password, $version);
        }
    ...
    public function trySubdomainUnauthenticatedGet()
    {
        ...
        if ($this->last_info['http_code'] == 302 || $this->last_info['http_code'] == 301) {
            // Do the NTLM POST to the redirect.
            $result = $this->doNTLMPost($this->last_response_headers['location']);
     ...
     *
     * @param string $path
     *            Path to a directory containing one or more CA
     *            certificates.
     * @return self
     */
     ...
     *            Overall cURL timeout for this request
     * @return boolean
     */
    ...
    /**
     * Reset the response-related structures.
     * Called before making a new
     * request.
     *
     * @return self
     */
